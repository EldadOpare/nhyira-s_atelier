  ...
  return (
    <div className="space-y-7 font-sans">
      ...
      <div className="flex justify-between items-center">
        <div>
          <h1 className="text-[22px] font-medium text-[#111827] tracking-tight">Portfolio</h1>
          <p className="text-[13px] text-[#9CA3AF] mt-0.5">Manage your published works</p>
        </div>
        <Dialog open={isModalOpen} onOpenChange={setIsModalOpen}>
          ...
          <DialogContent className="sm:max-w-[680px] max-h-[90vh] overflow-y-auto bg-white border border-[#EBEBEB] rounded-2xl p-0">
                ...
                <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-5">
                  <div className="grid grid-cols-2 gap-5">
                    <FormField control={form.control} name="title" render={({ field }) => (
                      <FormItem>
                        <FormLabel className="text-[11px] font-medium text-[#9CA3AF] uppercase tracking-wider">Title</FormLabel>
                    ...
                    <FormField control={form.control} name="category" render={({ field }) => (
                      <FormItem>
                        <FormLabel className="text-[11px] font-medium text-[#9CA3AF] uppercase tracking-wider">Category</FormLabel>
                        <FormControl><Input {...field} className="bg-white rounded-lg border-[#EBEBEB] focus-visible:ring-[#0D6E6E] h-9" /></FormControl>
                        <FormMessage />
                      </FormItem>
                    )} />
                  ...
                  <FormField control={form.control} name="description" render={({ field }) => (
                    <FormItem>
                      <FormLabel className="text-[11px] font-medium text-[#9CA3AF] uppercase tracking-wider">Description</FormLabel>
                  ...
                  <div className="grid grid-cols-2 gap-5">
                    <FormField control={form.control} name="packageDetails" render={({ field }) => (
                      <FormItem>
                        <FormLabel className="text-[11px] font-medium text-[#9CA3AF] uppercase tracking-wider">Package Details</FormLabel>
                    ...
                    <FormField control={form.control} name="estimatedBudget" render={({ field }) => (
                      <FormItem>
                        <FormLabel className="text-[11px] font-medium text-[#9CA3AF] uppercase tracking-wider">Estimated Budget</FormLabel>
                        <FormControl><Input {...field} className="bg-white rounded-lg border-[#EBEBEB] focus-visible:ring-[#0D6E6E] h-9" /></FormControl>
                        <FormMessage />
                      </FormItem>
                    )} />
                  ...
                  <div>
                    <p className="text-[11px] font-medium text-[#9CA3AF] uppercase tracking-wider mb-2.5">Images (URLs)</p>
                  ...
                  <div className="grid grid-cols-2 gap-5 items-end">
                    <FormField control={form.control} name="tags" render={({ field }) => (
                      <FormItem>
                        <FormLabel className="text-[11px] font-medium text-[#9CA3AF] uppercase tracking-wider">Tags (comma separated)</FormLabel>
                        <FormControl><Input {...field} className="bg-white rounded-lg border-[#EBEBEB] focus-visible:ring-[#0D6E6E] h-9" /></FormControl>
                        <FormMessage />
                      </FormItem>
                    )} />
        ...
        {tabs.map(tab => (
          <button key={tab.id} onClick={() => setActiveTab(tab.id as any)}
            className={`px-4 py-1.5 rounded-lg text-[12px] font-medium transition-colors ${
              activeTab === tab.id ? "bg-[#111827] text-white" : "text-[#9CA3AF] hover:bg-white hover:text-[#4B5563]"
            }`}>
            {tab.label}
          </button>
        ))}
      ...
      {isLoading ? (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5">
          {[1, 2, 3].map(i => <div key={i} className="h-[320px] bg-white border border-[#EBEBEB] rounded-xl animate-pulse" />)}
        </div>
      ) : !filteredItems?.length ? (
        <div className="text-center py-20 bg-white border border-[#EBEBEB] rounded-xl">
          <p className="text-[#B0B0B0] text-[13px]">No portfolio items found.</p>
        </div>
      ) : (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5">
          {filteredItems.map((item: any) => (
            <div key={item.id} className="bg-white border border-[#EBEBEB] rounded-xl overflow-hidden flex flex-col group hover:border-[#D8D8D8] transition-colors">
              ...
              <div className="p-4 flex-1 flex flex-col">
                <p className="text-[11px] font-medium text-[#B0B0B0] uppercase tracking-wider mb-1">{item.category}</p>
                <h3 className="text-[15px] font-medium text-[#111827] leading-snug">{item.title}</h3>
                <div className="mt-auto pt-4 flex justify-between items-center border-t border-[#F2F2F2]">
                  <span className="text-[11px] text-[#C4C4C4]">{format(new Date(item.createdAt), "MMM d, yyyy")}</span>
                  <div className="flex gap-1">
                    <Button variant="ghost" size="icon" className="h-7 w-7 rounded-lg hover:bg-[#F5F5F5] text-[#9CA3AF] hover:text-[#4B5563]" onClick={() => handleOpenModal(item)}>
                      <Edit2 size={13} />
                    </Button>
                    <Button variant="ghost" size="icon" className="h-7 w-7 rounded-lg hover:bg-red-50 text-[#C4C4C4] hover:text-red-500" onClick={() => handleDelete(item.id)}>
                      <Trash2 size={13} />
                    </Button>
                  </div>
                </div>
